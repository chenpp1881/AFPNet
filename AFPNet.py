...
class AFPNet(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.num_channel = opts.num_channel
        self.filter_sizes = len(opts.filter_sizes)
    ...
    def forward(self, x, opts):
        x, indecs = self.fpm(x, return_indecs=opts.return_indecs)
        x_res = x
        if opts.seq_model == 'Att':
        ...
        if opts.seq_model == 'RNN':
            x, _ = self.RNN(x)
        if opts.seq_model == 'LSTM':
            x, _ = self.LSTM(x)
        if opts.seq_model == 'GRU':
            x, _ = self.GRU(x)
        x = (x_res + x) / 2

        return F.sigmoid(self.output(x.reshape(-1, self.num_channel * (self.top_p + 1) * self.filter_sizes))), indecs
